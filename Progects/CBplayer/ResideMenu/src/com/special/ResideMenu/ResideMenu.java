...
public class ResideMenu extends FrameLayout{
    ...
    /**
     * 设置菜单点击回调接口 (注意此方法目前必须在添加完所有menuitem后才能调用)
     * @param onMenuItemClickListener
     */
    public void setOnMenuItemClickListener(OnMenuItemClickListener onMenuItemClickListener){
    	this.menuItemClickListener = onMenuItemClickListener;
    	int totalItems = leftMenuItems.size()+rightMenuItems.size();
    	if(totalItems>0){
    		if(leftMenuItems.size()>0){
    			int leftTotal = leftMenuItems.size();
    			for(int i=0;i<leftTotal;i++){
    				final int leftPos = i;
    				leftMenuItems.get(i).setOnClickListener(new OnClickListener() {
						@Override
						public void onClick(View v) {
							ResideMenu.this.menuItemClickListener.onMenuItemClicked(leftMenuItems.get(leftPos), leftPos, DIRECTION_LEFT);
    		...
    		if(rightMenuItems.size()>0){
    			int rightTotal = rightMenuItems.size();
    			for(int i=0;i<rightTotal;i++){
    				final int rightPos = i;
    				rightMenuItems.get(i).setOnClickListener(new OnClickListener() {
						@Override
						public void onClick(View v) {
							ResideMenu.this.menuItemClickListener.onMenuItemClicked(leftMenuItems.get(rightPos), rightPos, DIRECTION_RIGHT);
							ResideMenu.this.closeMenu();
						}
					});
    			}
    ...
    public interface OnMenuItemClickListener {
    	 ...
    	 * @param menuItem
    	 * @param position
    	 * @param direction  ResideMenu.DIRECTION_LEFT ResideMenu.DIRECTION_RIGHT
    	 */
